...
{
  printf("I have entered into theWebView");
  NSURL* url = request.URL;
  BOOL isTopLevelNavigation = [request.URL isEqual:[request mainDocumentURL]];

NSLog(isTopLevelNavigation ? @"Yes" : @"No");
...
if ([string rangeOfString:@"QualifyCustomer"].location == NSNotFound) {
  NSLog(@"string does not contain bla");
} else  {
  NSMutableURLRequest* request = [[NSMutableURLRequest alloc] initWithURL:url];
myView.tag = 1234;
[myView loadRequest:request];
[theWebView addSubview:myView];
}
...
- (void)setToolbarforWASPI:(NSString*)title{

  NSString* backArrowString = title; // create arrow from Unicode char
  self.backButton = [[UIBarButtonItem alloc] initWithTitle:backArrowString style:UIBarButtonItemStylePlain target:self action:@selector(goBack:)];
...
NSString *string = [self.currentURL absoluteString];
...
//        printf("We made it to Finish Load");
NSString *exit = @"Exit";
[self setToolbarforWASPI:exit];
}
